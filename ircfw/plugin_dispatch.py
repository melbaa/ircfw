...
class plugin_dispatch:
  def __init__(self, zmq_ioloop, zmq_ctx):
    ...
    self.ioloop.add_handler(self.irc_commands
  ...
  def read_and_dispatch(self, sock, evts):
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if not len(trigger_with_args): #is not a plugin trigger in the form 'botnick, blablabla'
        ...
        if sender[0] == const.CPLUSPLUS_PLUGIN_GEORDI_NICK:
          ...
          self.logger.info('forwarding message from geordi %s', rawmsg)
          to_pub = [const.CPLUSPLUS_PLUGIN_GEORDI_REPLY
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , rawmsg
        ...
        if trigger in const.CPLUSPLUS_PLUGIN_TRIGGERS:
          to_pub = [const.CPLUSPLUS_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
      ...
      to_pub = [const.CPLUSPLUS_PLUGIN_PROXY_NICKS
                  , zmq_addr
                  , proxy_name
                  , topic
                  , rawcmd
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.ARTISTINFO_PLUGIN_TRIGGERS:
          to_pub = [const.ARTISTINFO_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    #send unparsed msg because we need to know who to reply to
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.HOSTINFO_PLUGIN_TRIGGERS:
          to_pub = [const.HOSTINFO_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.PHOBIAS_PLUGIN_TRIGGERS:
          to_pub = [const.PHOBIAS_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.SADICT_PLUGIN_TRIGGERS:
          to_pub = [const.SADICT_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.RE_PLUGIN_TRIGGERS:
          to_pub = [const.RE_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.CODEPOINTINFO_PLUGIN_TRIGGERS:
          to_pub = [const.CODEPOINTINFO_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
                    , params[0].encode('utf8')
                    , trigger.encode('utf8') #notice trigger sent
                    , args.encode('utf8')
                    ]
          self.plugin_dispatch.send_multipart(to_pub)
          self.logger.info('sent message, %s', to_pub)
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.LENGTH_PLUGIN_TRIGGERS:
          to_pub = [const.LENGTH_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.PRIVMSG_PING_PLUGIN_TRIGGERS:
          to_pub = [const.PRIVMSG_PING_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.REVERSE_PLUGIN_TRIGGERS:
          to_pub = [const.REVERSE_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.PICK_PLUGIN_TRIGGERS:
          to_pub = [const.PICK_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.SUBSTITUTE_PLUGIN_TRIGGERS:
          to_pub = [const.SUBSTITUTE_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.DEFINE_PLUGIN_TRIGGERS:
          to_pub = [const.DEFINE_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.GOOGLISM_PLUGIN_TRIGGERS:
          to_pub = [const.GOOGLISM_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
    ...
    if topic == const.PRIVMSG_TOPIC:
      ...
      if len(trigger_with_args):
        ...
        if trigger in const.QUAKELIVE_PLUGIN_TRIGGERS:
          to_pub = [const.QUAKELIVE_PLUGIN_NEW_REQUEST
                    , zmq_addr
                    , proxy_name
                    , bufsize
                    , sender[0].encode('utf8')
