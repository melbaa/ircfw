...
class proxy:
  ...
  , zmq_ctx
    ...
    self.ioloop.add_handler(self.dealer
                            , self.on_reply_from_bot
                            , self.ioloop.READ)
    ...
    if len(should_see_nicks) > 0:
      ...
      delayedcb = zmq.eventloop.ioloop.DelayedCallback(
        lambda: self.dealer.send_multipart([const.CONTROL_MSG
                                            , self.proxyname
                                            ,const.PROXY_NICKS
                                            ,nicksbytes])
        , 5000
  ...
  def reconnect(self):
    ...
    try:
      ...
      self.ioloop.add_handler(self.irc_connection.irc_socket.fileno()
  ...
  def irc_connection_ready(self, fd, evts):
    ...
    def on_read(self):
      ...
      for rawmsg in raw_messages:
        ...
        """
        to_send = [const.IRC_MSG
                    , self.proxyname
                    #lame, have to fix in irc_connection
                    , current_irc_nick.encode('utf8')
                    , str(self.irc_connection.BUFSIZE).encode('utf8')
                    , rawmsg
                  ]
