...
class plugin:
  def __init__(self, zmq_ioloop, zmq_ctx):
    ...
    self.request.setsockopt(zmq.SUBSCRIBE, const.CPLUSPLUS_PLUGIN_NEW_REQUEST)
    self.request.setsockopt(zmq.SUBSCRIBE, const.CPLUSPLUS_PLUGIN_PROXY_NICKS)
    self.request.setsockopt(zmq.SUBSCRIBE, const.CPLUSPLUS_PLUGIN_GEORDI_REPLY)
  ...
  def on_request(self, sock, evts):
    ...
    if topic == const.CPLUSPLUS_PLUGIN_PROXY_NICKS:
      ...
      if (self.geordi_location is None) or (self.busy):
        ...
        replies = ircfw.unparse.make_privmsgs(senderbytes
                                                  , paramsbytes
                                                  , b"can't answer right now. try later"
        ...
        try:
          ...
          replies = ircfw.unparse.make_privmsgs(
            const.CPLUSPLUS_PLUGIN_GEORDI_NICK.encode('utf8')
            , const.CPLUSPLUS_PLUGIN_GEORDI_NICK.encode('utf8')
            , cppcode.encode('utf8')
            , int(bufsize.decode('utf8'))
            , 'raise'
          )
          self._send_replies(replies, self.geordi_location, proxy_name)
          ...
          replies = ircfw.unparse.make_privmsgs(
            senderbytes
            , paramsbytes
            , b"code too big. try to make it shorter"
            , int(bufsize.decode('utf8'))
            , 'multiline'
          )
          self._send_replies(replies, zmq_addr, proxy_name)
  ...
  def _on_timeout(self):
    if self.busy:
      ...
      replies = ircfw.unparse.make_privmsgs(
        senderbytes
        , paramsbytes
        , b'your request timed out. try later'
        , int(bufsize.decode('utf8'))
        , 'multiline'
      )
      self._send_replies(replies, zmq_addr, proxy_name)
